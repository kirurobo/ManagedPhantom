 ...
 *
 * Copyright(c) 2014-2019 Kirurobo
...
{
    static uint hHD = (uint)Hd.DeviceHandle.HD_INVALID_HANDLE;     // デバイスハンドル
    static List<Hd.SchedulerCallback> CallbackMethods;     // 参照が無くなるとGCされるので、メソッドを保持
    static private List<ulong> ScheduleHandles;                     // HDAPIDでスケジューリングした際のハンドルを保持
    static private Buttons CurrentButtons = Buttons.None;  // 現在のPHANTOMボタン押下状況
    static private Buttons LastButtons = Buttons.None; // 前回Update時のPHANTOMボタン

    /// <summary>
    /// PHANToMに接続できていればtrue
    /// </summary>
    /// <value><c>true</c> if this instance is avairable; otherwise, <c>false</c>.</value>
    static internal bool IsAvailable { get { return hHD != (uint)Hd.DeviceHandle.HD_INVALID_HANDLE; } }

    /// <summary>
    /// ジンバル部を基準としたペン先端座標 [mm] (PHANTOM座標系)
    /// </summary>
    static public Vector3 TipOffset = new Vector3(0.0f, 0.0f, -40.0f);

    /// <summary>
    /// 可動範囲下限 [mm]
    /// </summary>
    static public Vector3 WorkspaceMinimum { get; private set; }

    /// <summary>
    /// 可動範囲上限 [mm]
    /// </summary>
    static public Vector3 WorkspaceMaximum { get; private set; }

    /// <summary>
    /// 推奨可動範囲下限 [mm]
    /// </summary>
    static public Vector3 UsableWorkspaceMinimum { get; private set; }

    /// <summary>
    /// 推奨可動範囲上限 [mm]
    /// </summary>
    static public Vector3 UsableWorkspaceMaximum { get; private set; }

    /// <summary>
    /// 机の面に相当するY座標 [mm]
    /// </summary>
    static public float TableTopOffset { get; private set; }
    ...
    /// <summary>
    /// シングルトンにするため、インスタンスがあれば記憶
    /// </summary>
    private static PhantomManager _managerInstance;

    public static PhantomManager Instance
    {
        get
        {
            if (_managerInstance == null)
            {
                _managerInstance = new PhantomManager();
            }
            return _managerInstance;
        }
    }

    static public void Initialize(Hd.DeviceHandle handle = Hd.DeviceHandle.HD_DEFAULT_DEVICE)
    {
        IsRunning = false;

        // デフォルトのデバイスを準備
        hHD = Hd.hdInitDevice(handle);
        ...
        //PhantomManager.AddSchedule(() => { return true; });
    }
    ...
    /// 同期的に処理を呼び出します
    /// </summary>
    static public void Do(Callback callback)
    {
    ...
    static public void AddSchedule(Callback callback)
    {
    ...
    static private Hd.CallbackResult DoCallback(Callback callback)
    {
        if (!IsRunning)
        {
            return Hd.CallbackResult.HD_CALLBACK_DONE;
        }
    ...
    /// 登録済の非同期実行処理を全て消去します
    /// </summary>
    static public void ClearSchedule()
    {
        lock (ScheduleHandles)
        {
            for (int i = 0; i < ScheduleHandles.Count; i++)
            {
                ulong handle = ScheduleHandles[i];
                Hd.hdUnschedule(handle);
                ErrorCheck("Unschedule #" + handle.ToString());

                Debug.Log("Unscheduled ... " + handle);
                Hd.hdWaitForCompletion(handle, Hd.WaiteCode.HD_WAIT_INFINITE);
                Debug.Log("Unscheduled " + handle);
            }
    ...
    static public void SetSchedulerRate(uint rate)
    {
    ...
    static public Vector3 GetTipPosition()
    {
        double[] position = new double[3] { 0, 0, 0 };
    ...
    static public Buttons GetButton()
    {
    ...
    static public Buttons UpdateButtons()
    {
    ...
    /// <param name="button">Button.</param>
    static public bool GetButtonUp(Buttons button)
    {
    ...
    static public double GetSchedulerTimeStamp()
    {
    ...
    static public void SetForce(Vector3 force)
    {
    ...
    /// 可動範囲を取得
    /// </summary>
    static private void LoadWorkspaceLimit()
    {
